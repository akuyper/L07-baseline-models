...
Suppose each linear, elastic net, and nearest neighbor model takes 2 seconds to fit, while each random forest and boosted tree model takes 6 seconds.[^1] About how many minutes^[Report in additional unit(s) of time, if it would be more useful/impactful.] would it take to train all the models given the breakdown in @tbl-mod-totals-reg --- assuming we fit one model after the other (meaning fit sequentially)? 
...
[^1]: The time it takes to fit each model is hypothetical and does not reflect how long it really took to complete this process.
...
YOUR SOLUTION HERE --- suggest arranging assessment measures in a table for display; no code should be seen or accessible.
...
Suppose each logistic, elastic net, and nearest neighbor model takes 1 second to fit, while each random forest and boosted tree model takes 4 seconds.[^1] About how many minutes^[Report in additional unit(s) of time, if it would be more useful/impactful.] would it take to train all the models given the breakdown in @tbl-mod-totals-class --- assuming we fit one model after the other (meaning fit sequentially)? 
